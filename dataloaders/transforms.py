...
class RandomCrop(object):
    ...
    def __init__(self, dims):
        ...
        """
        self.dims = dims
        self.i = np.random.uniform(0, 1)
        self.j = np.random.uniform(0, 1)

    @staticmethod
    def get_params(img, dims, rand_i, rand_j):
        ...
        assert w <= img_w, "cannot crop {} to size {}".format(img.shape, (h, w))
        assert h <= img_h, "cannot crop {} to size {}".format(img.shape, (h, w))
        
        i = int((img_h - h) * rand_i)
        j = int((img_w - w) * rand_j)
        return i, j
    ...
    def __call__(self, img):
        ...
        i, j = self.get_params(img, self.dims, self.i, self.j)
        (h, w) = self.dims
    ...
    def __repr__(self):
        return self.__class__.__name__ + '(i={0},j={1})'.format(
            self.i, self.j)